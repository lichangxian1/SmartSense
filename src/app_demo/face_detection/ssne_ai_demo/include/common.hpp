...
// 2. 新增：GCNv2 推理核心类
class GCNV2_EXTRACTOR {
  public:
    std::string ModelName() const { return "gcnv2"; }

    // 初始化模型
    void Initialize(std::string& model_path, std::array<int, 2>* in_det_shape);
    
    // 执行推理
    void Predict(ssne_tensor_t* img_in, GcnResult* result, float conf_threshold = 0.015f);
  ...
  private:
    uint16_t model_id = 0;
    ssne_tensor_t inputs[1];
    ssne_tensor_t outputs[2]; // GCNv2 只有 2 个输出：prob 和 desc
    
    AiPreprocessPipe pipe_offline; // 官方的硬件级预处理管道
    std::array<int, 2> det_shape;  // 我们这里是 {640, 480}

    void Postprocess(GcnResult* result, float conf_threshold);
};
